# AGENTS.md — Contexto do Projeto fortbc

> Este arquivo é lido automaticamente pelo Codex ao iniciar. Mantém o contexto do projeto entre sessões. Atualize-o quando decisões importantes mudarem.
...
3. Abrir o Codex apontando para a pasta — este AGENTS.md carrega o contexto automaticamente.

**Sempre atualizar este arquivo ao final de cada sessão** com decisões, mudanças de mecânica e backlog atualizado.

## Atualização de sessão — 2026-06-18 — Expansão de cartas CODEX

- Sessão focada em planejamento/planilha de novas cartas, sem alterar `index.html`, `GAME_DATA.csv` ou `FUSIONS.csv` originais.
- Criados arquivos experimentais:
  - `GAME_DATA_CODEX.csv` — base experimental com 74 cartas originais + 52 novas = 126 linhas, mantendo o schema atual de 19 colunas.
  - `FUSIONS_CODEX.csv` — receitas novas/planejadas de fusão.
  - `GAME_DATA_CODEX_NOTES.md` — notas curtas sobre o que já funciona no engine e o que precisa de código.
  - `outputs/game_data_codex/GAME_DATA_CODEX.xlsx` — versão em planilha para revisão.
  - `RELATORIO_HANDOFF_CARTAS_CODEX.md` — relatório completo para continuar em outro Codex/máquina.
- Conclusão técnica: só adicionar ao CSV é suficiente para `pawn`, `hero`, `fusion`, `item` e `trap` usando efeitos já existentes (`atk_buff`, `draw2`, `resurrect`, `debuff_*`, `trap_*`). Não é suficiente ainda para `field`, `equip`, tags/arquétipos ou economia/recurso.
- Próxima decisão importante: definir se economia/recurso vira mana real ou se será representada por compra extra, desconto de custo e/ou ação rápida extra. Recomendação atual: começar com economia leve.
